...
<script lang="ts" setup>
  ...
  const { t } = useI18n();
  const listeningTest = ref<ListeningPart[]>(listeningParts);
  ...
  function handleUpdateQuestion(partIdx: number, value: QuestionItem) {
    const questionIndex = listeningTest.value[partIdx].questions.findIndex(
      (item) => item.no === questionCurrent.value?.no,
    );
    if (questionIndex !== -1) {
      listeningTest.value[partIdx].questions[questionIndex] = value;
    }
